  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        admin_email: community.adminEmail
      }])
      .select()
      .single()
...
export async function updateCommunity(id: string, updates: Partial<Community>): Promise<Community | null> {
  try {
    const { members, ...communityUpdates } = updates
    
    const updateData: any = { ...communityUpdates }
    if (updates.adminName) updateData.admin_name = updates.adminName
